          ...
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600">
                Created on {formatDate(purchase.createdAt)} • Required by {formatDate(purchase.estimatedDate)}
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-6">
                {purchase.materials.map((material, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-6 bg-gray-50">
                    ...
                    {/* Dimensions */}
                    {material.materialId.hasDimensions && material.materialId.dimensions.length > 0 && (
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-600 mb-2">
                          Dimensions & Area
                        </label>
              ...
              <div className="space-y-3">
                ...
                {purchase.status.toLowerCase() === 'pending' && (
                  <Button 
                    variant="success" 
                    className="w-full"
                    onClick={() => router.push(`/dashboard/planning/purchase/${purchase._id}/fulfill`)}
                  >
                    Fulfill Requirement
                  </Button>
                )}
                {purchase.status.toLowerCase() === 'verification needed' && (
                  <Button 
                    variant="primary" 
                    className="w-full"
                    onClick={() => router.push(`/dashboard/planning/purchase/${purchase._id}/verify`)}
                  >
                    Verify Purchase
                  </Button>
                )}
                  ...
                  <div key={index} className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        ...
                        <p>By: {entry.user_name} ({entry.user_email})</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Materials Count:</span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-gray-900">
                    ₹{purchase.materials.reduce((total, material) => 
                      total + (material.quantity * material.materialId.unit_cost), 0
                    ).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Documents:</span>
                  <span className="font-medium text-gray-900">{purchase.documents?.length || 0}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Vendors:</span>
                  <span className="font-medium text-gray-900">
                    {new Set(purchase.materials.map(m => m.vendorId._id)).size}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">High Priority Items:</span>
                  <span className="font-medium text-gray-900">
                    {purchase.materials.filter(m => m.criticality.toLowerCase() === 'high').length}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Status Updates:</span>
                  <span className="font-medium text-gray-900">{purchase.timeline.length}</span>
                </div>
              </div>
            </div>
          </div>
