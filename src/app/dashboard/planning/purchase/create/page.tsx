...
const CreatePurchasePage: React.FC = () => {
  ...
  const handleCreatePurchase = async (data: PurchaseFormData): Promise<void> => {
    setLoading(true);
    setError("");
    setSuccess("");
    ...
    try {
      ...
      const purchaseData = {
        materials: data.materials.map((item) => ({
          ...
          criticality: item.criticality || "Regular", // Include criticality
          expected_days: Number(item.expected_days) || 7, // Include expected days
        })),
      ...
      const response = await PurchaseReqApis.createPurchaseRequirement(purchaseData);
      ...
      console.error("Error creating purchase requirement:", error);
      setError(
        error.response?.data?.message ||
          error.message ||
          "An error occurred while creating the purchase requirement. Please try again."
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-center space-x-4 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Fill in the details below to create a new purchase requirement for raw materials
              </p>
            </div>
          </div>
        </div>
