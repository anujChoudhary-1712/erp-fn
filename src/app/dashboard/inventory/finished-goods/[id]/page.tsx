      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{finishedGood.product_name}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <p className="mt-1 text-gray-900">{finishedGood.description || "No description provided"}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-500">Unit</label>
                      <p className="mt-1 text-gray-900 capitalize">{finishedGood.unit}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      ...
                      <p className="mt-1 text-2xl font-bold text-gray-900">{formatCurrency(finishedGood.unit_price)}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-500">Current Stock</label>
                      <p className="mt-1 text-2xl font-bold text-gray-900">{finishedGood.current_stock}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-500">Trigger Value</label>
                      <p className="mt-1 text-2xl font-bold text-gray-900">{finishedGood.trigger_value}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <p className="mt-1 text-gray-900">{finishedGood.created_by.name}</p>
                      <p className="text-sm text-gray-500">{finishedGood.created_by.email}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-500">Created Date</label>
                      <p className="mt-1 text-gray-900">{formatDate(finishedGood.createdAt)}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-gray-900">Production Workflows</h2>
                <button
              ...
              {finishedGood.workflows.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500">No workflows defined for this product</p>
                </div>
              ) : (
                <div className="space-y-6">
                  {finishedGood.workflows.map((workflow, index) => (
                    <div key={workflow._id} className="border border-gray-200 rounded-lg p-4">
                      <h3 className="font-semibold text-gray-900 mb-4">{workflow.workflow_name}</h3>
                      
                      <div className="space-y-4">
                        {workflow.stages.map((stage) => (
                          <div key={stage.stage_id} className="pl-4 border-l-2 border-blue-200">
                            ...
                            {stage.quality_check_required && (
                              <div className="mt-2">
                                ...
                                {stage.quality_parameters && stage.quality_parameters.length > 0 && (
                                  <div className="mt-2">
                                    <p className="text-sm font-medium text-gray-700 mb-1">Quality Parameters:</p>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                      {stage.quality_parameters.map((param, paramIndex) => (
                                        <div key={paramIndex} className="text-sm bg-gray-50 p-2 rounded">
          ...
          <div className="space-y-8">
            <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
              <h2 className="text-xl font-semibold text-gray-900 mb-6">Raw Materials Used</h2>
                    ...
                    <div key={material._id} className="border border-gray-200 rounded-lg p-4">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{material.stock_used} {material.unit}</span>
                        </div>
                        <div className="flex justify-between">
                      ...
                      {material.verificationParameters.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <p className="text-sm font-medium text-gray-700 mb-2">Verification Parameters:</p>
                          <div className="space-y-1">
                            {material.verificationParameters.map((param) => (
                              <div key={param._id} className="text-xs bg-gray-50 p-2 rounded">
                                <span className="font-medium">{param.parameter}:</span> {param.specificationValue}
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {material.wastage_records && material.wastage_records.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          <p className="text-sm font-medium text-gray-700 mb-2">Wastage Records:</p>
                          <div className="space-y-1">
                            {material.wastage_records.map((record) => (
                              <div key={record._id} className="text-xs bg-red-50 p-2 rounded">
                                <div className="flex justify-between">
                                  <span>{formatDate(record.checking_date)}</span>
                                  <span className="font-medium">{record.quantity_wasted} wasted</span>
                                </div>
                                <div className="text-gray-600 mt-1">{record.reason_for_wastage}</div>
                              </div>
                            ))}
                          </div>
                        </div>
        ...
        {showAddWorkflowModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-semibold text-gray-900">Add New Workflow</h2>
                  <button
                    onClick={() => {
                      setShowAddWorkflowModal(false);
                      setWorkflowError("");
