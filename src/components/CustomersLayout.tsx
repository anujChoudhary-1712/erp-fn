...
const CustomersLayout: React.FC<CustomersLayoutProps> = ({
  ...
  const { user, isLoading } = useUser(); // Now we're getting isLoading from context
  ...
  const isCurrentPathAccessible = () => {
    ...
    const findItemByPath = (items: typeof allNavigationItems): any => {
      ...
      const allRoutes: Array<{path: string, item: any, type: 'parent' | 'child', parent?: any}> = [];
      ...
      if (!bestMatch) {
        ...
        for (const route of allRoutes) {
          ...
          console.log(`Checking prefix: "${pathname}".startsWith("${route.path}")`);
    ...
    console.log(`Checking access for ${currentItem.id} with required roles:`, currentItem.requiredRoles);
